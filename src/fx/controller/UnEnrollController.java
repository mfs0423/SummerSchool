 ...
 * @author Meng
 */
public class UnEnrollController implements Initializable {
    ...
    private void showList(ActionEvent event) {
    	...
    	if("Confirm".equals(btnStr)){
            ...
            courses = DataService.getCoursesByName(s.getFirstName(), s.getLastName());
    ...
    @FXML
    private void unenroll(ActionEvent event) {
    	String btnStr = btn.getText();
    	if("Confirm".equals(btnStr)){
		...
		int update = DataService.removeStudentCourse(s.getId(),course.getId());
        if(update==1){
